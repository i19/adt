...
func ssnIHelper(start int, state *[]int, nums []int, target int, res *[][]int) {
	sum := 0
	for _, v := range *state {
		sum += v
	}
	...
	for i := start; i < len(nums); i++ {
		...
		*state = append(*state, nums[i])
		// 尝试求解
		ssnIHelper(i, state, nums, target, res)
		// 状态回溯
...
func ssnI(nums []int, target int) [][]int {
	...
	res := [][]int{}
	start := 0
	ssnIHelper(start, &state, nums, target, &res)
	return res
}
...
func ssnIIHelper(start int, state *[]int, nums []int, target int, res *[][]int) {
	sum := 0
	for _, v := range *state {
		sum += v
	}
	...
	for i := start; i < len(nums); i++ {
		if _, ok := duplicate[nums[i]]; !ok {
			...
			*state = append(*state, nums[i])
			// 尝试求解
			ssnIIHelper(i, state, nums, target, res)
			// 状态回溯
			*state = (*state)[:len(*state)-1]
		}
	}
